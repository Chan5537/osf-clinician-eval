...
// (sleepfm-agent-eval: scripts/export_demo_cases.py / gen_demo_cases_json.py) renders each
// arm's letter, embeds the risk figure as a base64 data-URI, shuffles display order per case,
// and writes demo-cases.generated.json. The app cycles through every entry in DEMO_CASES.
//
// SOURCE-BLINDING RULE: each response's `markdown` must never contain the strings Agent, Base,
// GPT, Gemini, SleepFM, tool, ReAct, oracle, or "ground truth". The arm identity lives only in
